...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion, is_test=False):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
            ...
            batch_y_norm = batch_y_norm[:, -horizon:, :]
            loss_elem = criterion(outputs, batch_y_norm)
            loss_by_var = loss_elem.mean(dim=(0,1))
            loss = loss_by_var.sum().detach().cpu()
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                #         loss_elem = criterion(outputs_, batch_y_)
                #         loss_by_var = loss_elem.mean(dim=(0,1))
                #         total_loss = loss_by_var.sum()
                #         loss = total_loss
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        # --- base loss（変量方向は平均しない） ---
                        loss_elem   = criterion(outputs_, batch_y_)      # [B,H,C]
                        loss_by_var = loss_elem.mean(dim=(0,1))          # [C]
                        base_loss   = loss_by_var.sum()

                        # --- A: masked cross-prediction（損失だけマスク）---
                        B, H, C   = outputs_.shape
                        p_mask    = getattr(self.args, "mask_prob", 0.2)     # 例: --mask_prob 0.2
                        lam_rec   = getattr(self.args, "lambda_rec", 1e-3)   # 例: --lambda_rec 1e-3

                        # 変量単位マスク（時間方向に一貫させる）: [B,1,C] をHにブロードキャストして使う
                        var_mask  = (torch.rand(B, 1, C, device=outputs_.device) < p_mask).float()

                        # 再構成損失（マスク箇所のみのMSE）
                        rec_elem   = ((outputs_ - batch_y_)**2) * var_mask   # [B,H,C]
                        rec_by_var = rec_elem.mean(dim=(0,1))                # [C]
                        # マスク有効数で正規化（/H で時刻数の差を除去）
                        valid_cnt  = (var_mask.sum() / H).clamp_min(1.0)
                        rec_loss   = rec_by_var.sum() / valid_cnt

                        total_loss = base_loss + lam_rec * rec_loss
                        loss = total_loss
                    scaler.scale(loss).backward()
                    scaler.step(model_optim)
                    scaler.update()
    ...
    def test(self, setting, test=0):
        ...
        # （可視化用）元スケールも保持したいならコメントアウトを外す
        # preds_raw_all, trues_raw_all = [], []
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # ★ ここが再現ポイント：標準化スケールに合わせて metric を計算
                ...
                # （可視化/参考）元スケールも保持したいならコメントアウト
                # preds_raw_all.append(pred_y.detach().cpu())
                # trues_raw_all.append(true_y.detach().cpu())
        ...
        # 参考：元スケールでも見たい場合はコメントアウトを外す
        # preds_raw = torch.cat(preds_raw_all, dim=0).numpy()
        # trues_raw = torch.cat(trues_raw_all, dim=0).numpy()
        # mae_raw, mse_raw, rmse_raw, mape_raw, mspe_raw = metric(preds_raw, trues_raw)
        # print('[RAW scale] mse:{}, mae:{}'.format(mse_raw, mae_raw))
